...
```bash
ls -lah ~/CivilAI/agents/reports/server
```
...
```bash
./agents/run_agents_server.sh frontend
```

Run one agent:

```bash
./agents/run_agents_server.sh security
./agents/run_agents_server.sh deployment-gate
./agents/run_agents_server.sh server-connections
./agents/run_agents_server.sh server-runtime
./agents/run_agents_server.sh data-leak
./agents/run_agents_server.sh tenant-isolation
./agents/run_agents_server.sh llm-safety
```

Run LLM safety probes intentionally:

```bash
CIVILAI_RUN_LLM_SAFETY=true ./agents/run_agents_server.sh llm-safety
```
...
```bash
cd ~/CivilAI
chmod +x ./agents/install_server_timer.sh ./agents/uninstall_server_timer.sh ./agents/run_agents_server.sh
...
```bash
grep -R '"status": "fail"' agents/reports/server/*.json -n
```

Show warnings:

```bash
grep -R '"status": "warn"' agents/reports/server/*.json -n
```
...
for path in sorted(Path("agents/reports/server").glob("*.json")):
...
```bash
cat agents/reports/server/security.json
cat agents/reports/server/deployment-gate.json
cat agents/reports/server/server-connections.json
```
...
sudo rm -rf /home/will/CivilAI/agents/reports/server
mkdir -p /home/will/CivilAI/agents/reports/server
sudo chown -R will:will /home/will/CivilAI/agents/reports
...
systemctl status civilai-agents.service --no-pager -l
journalctl -u civilai-agents.service -n 120 --no-pager
ls -lah /home/will/CivilAI/agents/reports/server
```
...
CIVILAI_AGENT_REPORT_DIR=/home/will/CivilAI/agents/reports/server
...
```bash
sudo mkdir -p /home/will/CivilAI/agents/reports/server
sudo chown -R will:will /home/will/CivilAI/agents/reports
```
