...
class SecurityAgent(BaseAgent):
    name = "security"
    description = (
        "Security checks for secrets, auth guardrails, headers, and dependency audit support."
    )
    ...
    def _check_jwt_secret_configuration(self) -> CheckResult:
        ...
        if "require_production_secret" in text and '"JWT_SECRET_KEY"' in text:
            return self.pass_result(
                "jwt-secret",
                "JWT secret is configurable and required in production/server environments.",
            )
    ...
    def _check_password_policy(self) -> CheckResult:
        ...
        if missing:
            return self.fail_result(
                "password-policy",
                "Password policy is missing expected strength checks.",
                missing=missing,
            )
        return self.pass_result(
            "password-policy", "Password policy requires length, uppercase, lowercase, and number."
        )
    ...
    def _check_security_headers(self) -> CheckResult:
        try:
            ...
            return self.skip_result(
                "security-headers",
                "Backend is not reachable; runtime header check skipped.",
                error=str(exc),
            )
        ...
        missing = {
            header: value for header, value in expected.items() if response.header(header) != value
        }
        if missing:
            return self.fail_result(
                "security-headers",
                "Backend response is missing expected security headers.",
                missing=missing,
            )
    ...
    def _check_protected_route_requires_auth(self) -> CheckResult:
        try:
            ...
            return self.skip_result(
                "auth-guard",
                "Backend is not reachable; protected route check skipped.",
                error=str(exc),
            )

        if response.status == 401:
            return self.pass_result(
                "auth-guard", "Protected auth route rejects unauthenticated requests."
            )
        return self.fail_result(
            "auth-guard",
            "Protected auth route did not reject unauthenticated request.",
    ...
    def _check_dependency_audit(self) -> CheckResult:
        ...
        executable = next(
            (str(path) for path in python_candidates if path.exists()), sys.executable
        )
        result = run_command(
            [executable, "-m", "pip_audit", "-r", "requirements.txt"],
            self.repo_root,
            timeout_seconds=180,
        )
        ...
        if result.returncode == 0:
            return self.pass_result(
                "dependency-audit", "pip-audit completed without known vulnerabilities."
            )
        ...
        return self.fail_result(
            "dependency-audit", "pip-audit reported issues or failed.", output=combined[-4000:]
        )
