...
class FrontendFeaturesAgent(BaseAgent):
    ...
    def _check_auth_experience_modes(self) -> CheckResult:
        ...
        if missing:
            return self.fail_result("auth-experience", "Auth experience is missing expected login/register/guest pieces.", missing=missing)
        return self.pass_result("auth-experience", "Login, register, guest mode, and redirect intent are represented.")
    ...
    def _check_protected_workspace_routes(self) -> CheckResult:
        ...
        return self.pass_result("protected-routes", "Protected route wrapper and workspace pages exist.")
    ...
    def _check_chat_api_integrations(self) -> CheckResult:
        ...
        if missing:
            return self.fail_result("chat-integrations", "Chat workspace is missing expected backend integrations.", missing=missing)
        return self.pass_result("chat-integrations", "Chat workspace calls jurisdiction, query, upload, jobs, and saved-chat APIs.")

    def _check_guest_mode_storage(self) -> CheckResult:
        auth_context = self._read("app/context/AuthContext.tsx")
        chat_page = self._read("app/chat/page.tsx")
        expected = ["GUEST_SESSION_KEY", "civilai_guest_threads_", "civilai_guest_active_thread_", "localStorage"]
        combined = f"{auth_context}\n{chat_page}"
        missing = [item for item in expected if item not in combined]
        if missing:
            return self.warn_result("guest-mode", "Guest mode persistence may be incomplete.", missing=missing)
        return self.pass_result("guest-mode", "Guest session and guest chat persistence are represented.")

    def _check_account_features(self) -> CheckResult:
        account_page = self._read("app/account/page.tsx")
        expected = ["updateProfile", "CUSTOM_API_BASE", "/jurisdictions", "jurisdiction", "full_name"]
        missing = [item for item in expected if item not in account_page]
        if missing:
            return self.fail_result("account-features", "Account page is missing expected profile features.", missing=missing)
        return self.pass_result("account-features", "Account page supports profile refresh and updates.")
    ...
    def _check_subscription_features(self) -> CheckResult:
        ...
        if missing:
            return self.warn_result("subscription-features", "Subscription page may be missing expected subscription display pieces.", missing=missing)
        ...
        return self.pass_result("subscription-features", "Subscription page includes subscription/usage integration.")
    ...
    def _check_home_to_chat_intent(self) -> CheckResult:
        ...
        missing = [item for item in expected if item not in combined]
        if missing:
            return self.warn_result("home-chat-intent", "Home-to-chat prompt handoff may be incomplete.", missing=missing)
        return self.pass_result("home-chat-intent", "Home page prompt handoff into chat is represented.")
