...
class DeploymentGateAgent(BaseAgent):
    ...
    def _check_jwt_secret_enforced(self) -> CheckResult:
        auth = (self.repo_root / "backend" / "app" / "auth.py").read_text(encoding="utf-8", errors="replace")
        if "require_production_secret(\"JWT_SECRET_KEY\"" in auth:
            return self.pass_result("gate-jwt-secret", "JWT_SECRET_KEY is required in production/server environments.")
        return self.fail_result("gate-jwt-secret", "JWT_SECRET_KEY is not enforced for production/server environments.")
    ...
    def _check_env_example_documents_secret(self) -> CheckResult:
        ...
        return self.warn_result("gate-env-example", ".env.example should document JWT_SECRET_KEY for production.")

    def _check_backend_health(self) -> CheckResult:
        try:
            response = request("GET", f"{self.context.backend_url}/health")
        except ConnectionError as exc:
            return self.skip_result("gate-backend-health", "Backend is not reachable from this environment.", error=str(exc))
        if response.status == 200 and response.json().get("status") == "ok":
            return self.pass_result("gate-backend-health", "Backend health is ok.")
        return self.fail_result("gate-backend-health", "Backend health is not ok.", status=response.status)
    ...
    def _check_cors_not_wildcard_static(self) -> CheckResult:
        config = (self.repo_root / "backend" / "app" / "core" / "config.py").read_text(encoding="utf-8", errors="replace")
    ...
    def _check_systemd_template_not_placeholder(self) -> CheckResult:
        service = (self.repo_root / "deploy" / "civilai-backend.service").read_text(encoding="utf-8", errors="replace")
        if "YOUR_LINUX_USER" in service:
            return self.warn_result("gate-systemd-template", "Systemd template still contains YOUR_LINUX_USER; replace on the server before release.")
