...
class ServerConnectionsAgent(BaseAgent):
    ...
    def _check_backend_health(self, results: list[CheckResult]) -> bool:
        try:
            ...
            results.append(
                self.fail_result(
                    "server-backend-health", "Backend server is not reachable.", error=str(exc)
                )
            )
            return False
        ...
        if response.status != 200:
            results.append(
                self.fail_result(
                    ...
                    "Backend health returned unexpected status.",
        ...
        if data.get("status") != "ok":
            results.append(
                self.fail_result(
                    "server-backend-health", "Backend health payload is not ok.", payload=data
                )
            )
            return False
    ...
    def _check_v1_health_runtime_settings(self) -> CheckResult:
        try:
            ...
            return self.fail_result(
                "server-v1-health",
                "v1 health endpoint timed out or was unreachable.",
                error=str(exc),
            )
        if response.status != 200:
            return self.fail_result(
                "server-v1-health", "v1 health endpoint is not reachable.", status=response.status
            )
        ...
        expected_fields = [
            ...
            "async_ingestion_enabled",
        ]
        missing = [field for field in expected_fields if field not in data]
        if missing:
            return self.fail_result(
                "server-v1-health", "v1 health is missing runtime settings.", missing=missing
            )
        return self.pass_result(
            "server-v1-health",
            "v1 health exposes expected runtime settings.",
            settings={field: data[field] for field in expected_fields},
        )
    ...
    def _check_custom_jurisdictions(self) -> CheckResult:
        try:
            ...
            return self.warn_result(
                ...
                "Custom jurisdictions endpoint timed out or was unreachable.",
                error=str(exc),
            )
        if response.status != 200:
            return self.fail_result(
                "custom-jurisdictions",
                "Custom jurisdictions endpoint failed.",
                status=response.status,
            )
        data = response.json()
        ...
        if not isinstance(jurisdictions, list):
            return self.fail_result(
                "custom-jurisdictions", "Jurisdictions payload is not a list.", payload=data
            )
        if not jurisdictions:
            return self.warn_result(
                "custom-jurisdictions",
                "Jurisdictions endpoint works but returned no indexed jurisdictions.",
            )
        return self.pass_result(
            "custom-jurisdictions",
            "Jurisdictions endpoint returns indexed options.",
    ...
    def _check_custom_navigation_map(self) -> CheckResult:
        try:
            ...
            return self.warn_result(
                ...
                "Navigation map endpoint timed out or was unreachable.",
                error=str(exc),
            )
        if response.status != 200:
            return self.fail_result(
                "custom-navigation-map", "Navigation map endpoint failed.", status=response.status
            )
        data = response.json()
        if "documents" not in data:
            return self.warn_result(
                "custom-navigation-map",
                "Navigation map responded but did not include documents.",
                keys=sorted(data.keys()),
            )
        return self.pass_result(
            "custom-navigation-map",
            "Navigation map endpoint returns document structure.",
    ...
    def _check_metrics_endpoint(self) -> CheckResult:
        try:
            ...
            return self.warn_result(
                "metrics", "Metrics endpoint timed out or was unreachable.", error=str(exc)
            )
        ...
        if response.status == 503:
            return self.warn_result(
                "metrics", "Metrics endpoint exists but prometheus-client is unavailable."
            )
        return self.fail_result(
            "metrics", "Metrics endpoint returned unexpected response.", status=response.status
        )
    ...
    def _check_cors_preflight(self) -> CheckResult:
        try:
            ...
            return self.fail_result(
                "cors-preflight", "CORS preflight timed out or was unreachable.", error=str(exc)
            )
        if response.status not in {200, 204}:
            return self.fail_result(
                "cors-preflight",
                "CORS preflight failed for configured frontend URL.",
                status=response.status,
            )
        allow_origin = response.header("Access-Control-Allow-Origin")
        if not allow_origin:
            return self.fail_result(
                "cors-preflight", "CORS preflight did not return Access-Control-Allow-Origin."
            )
        return self.pass_result(
            "cors-preflight", "CORS accepts the configured frontend URL.", allow_origin=allow_origin
        )
    ...
    def _check_auth_database_write_path(self) -> CheckResult:
        try:
            ...
            return self.fail_result(
                "auth-db-route",
                "Auth validation route timed out or was unreachable.",
                error=str(exc),
            )
        if response.status == 422:
            return self.pass_result(
                "auth-db-route", "Auth route is mounted and validation works before writes."
            )
        return self.fail_result(
            "auth-db-route",
            "Auth validation route returned unexpected status.",
            status=response.status,
        )
    ...
    def _check_frontend_reachable(self) -> CheckResult:
        try:
            ...
            return self.skip_result(
                "frontend-public-url",
                "Frontend URL is not reachable from this environment.",
                error=str(exc),
            )

        if response.status == 200:
            return self.pass_result(
                "frontend-public-url", "Frontend URL is reachable from the server test environment."
            )
        return self.warn_result(
            "frontend-public-url",
            "Frontend URL returned unexpected status.",
            status=response.status,
        )
