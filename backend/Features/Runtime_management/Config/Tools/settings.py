...
class CivilAISettings:
    ...
    backend_root: Path = field(default_factory=lambda: Path(__file__).resolve().parents[4])
    environment: str = field(default_factory=lambda: os.getenv("ENVIRONMENT", "development"))
    database_url: str = field(
        default_factory=lambda: os.getenv(
            "DATABASE_URL",
            f"sqlite:///{(Path(__file__).resolve().parents[4] / 'Data' / 'civilai_auth.db').as_posix()}",
        )
    )
    structured_database_url: str = field(
        default_factory=lambda: os.getenv(
            "STRUCTURED_DATABASE_URL",
            f"sqlite:///{(Path(__file__).resolve().parents[4] / 'Data' / 'civilai_structured.db').as_posix()}",
        )
    )
    ...
    max_request_bytes: int = field(
        default_factory=lambda: int(os.getenv("MAX_REQUEST_BYTES", str(60 * 1024 * 1024)))
    )
    rate_limit_per_minute: int = field(
        default_factory=lambda: int(os.getenv("RATE_LIMIT_PER_MINUTE", "120"))
    )
    free_monthly_token_limit: int = field(
        default_factory=lambda: int(os.getenv("FREE_MONTHLY_TOKEN_LIMIT", "0"))
    )
    standard_monthly_token_limit: int = field(
        default_factory=lambda: int(os.getenv("STANDARD_MONTHLY_TOKEN_LIMIT", "0"))
    )
    pro_monthly_token_limit: int = field(
        default_factory=lambda: int(os.getenv("PRO_MONTHLY_TOKEN_LIMIT", "0"))
    )
    free_monthly_message_limit: int = field(
        default_factory=lambda: int(os.getenv("FREE_MONTHLY_MESSAGE_LIMIT", "0"))
    )
    pro_monthly_message_limit: int = field(
        default_factory=lambda: int(os.getenv("PRO_MONTHLY_MESSAGE_LIMIT", "0"))
    )

    ai_default_provider: str = field(
        default_factory=lambda: os.getenv("AI_DEFAULT_PROVIDER", "openai")
    )
    ai_fallback_provider: str = field(
        default_factory=lambda: os.getenv("AI_FALLBACK_PROVIDER", "deepseek")
    )
    ai_local_provider: str = field(default_factory=lambda: os.getenv("AI_LOCAL_PROVIDER", "ollama"))
    ai_embedding_provider: str = field(
        default_factory=lambda: os.getenv("AI_EMBEDDING_PROVIDER", "openai")
    )
    ai_background_provider: str = field(
        default_factory=lambda: os.getenv("AI_BACKGROUND_PROVIDER", "deepseek")
    )
    ai_request_timeout_seconds: float = field(
        default_factory=lambda: float(os.getenv("AI_REQUEST_TIMEOUT_SECONDS", "60"))
    )
    ai_max_retries: int = field(default_factory=lambda: int(os.getenv("AI_MAX_RETRIES", "2")))
    ai_enable_ollama_fallback: bool = field(
        default_factory=lambda: _bool_env("AI_ENABLE_OLLAMA_FALLBACK", True)
    )
    ...
    deepseek_reasoner_model: str = field(
        default_factory=lambda: os.getenv("DEEPSEEK_REASONER_MODEL", "deepseek-reasoner")
    )
    ...
    async_ingestion_enabled: bool = field(
        default_factory=lambda: _bool_env("ASYNC_INGESTION_ENABLED", False)
    )
    ...
    qdrant_timeout_seconds: float = field(
        default_factory=lambda: float(os.getenv("QDRANT_TIMEOUT_SECONDS", "300"))
    )
    qdrant_upsert_batch_size: int = field(
        default_factory=lambda: int(os.getenv("QDRANT_UPSERT_BATCH_SIZE", "64"))
    )
    vector_store_backend: str = field(
        default_factory=lambda: os.getenv("VECTOR_STORE_BACKEND", "chroma")
    )
    chroma_persist_directory: str = field(
        default_factory=lambda: os.getenv("CHROMA_PERSIST_DIRECTORY", "backend/Data/chroma_db")
    )

    s3_endpoint_url: str | None = field(
        default_factory=lambda: os.getenv("S3_ENDPOINT_URL") or os.getenv("R2_ENDPOINT_URL")
    )
    s3_bucket_name: str | None = field(
        default_factory=lambda: os.getenv("S3_BUCKET_NAME") or os.getenv("R2_BUCKET_NAME")
    )
    s3_access_key_id: str | None = field(
        default_factory=lambda: os.getenv("S3_ACCESS_KEY_ID") or os.getenv("R2_ACCESS_KEY_ID")
    )
    ...
    local_upload_directory: str = field(
        default_factory=lambda: os.getenv("LOCAL_UPLOAD_DIRECTORY", "backend/Data/PDF")
    )
