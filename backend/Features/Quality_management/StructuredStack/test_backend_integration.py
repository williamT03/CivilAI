...
from backend.Features.RAG_management import rag_api as app_custom
from backend.Features.Runtime_management import backend_run

# The FastAPI app imports `backend.Features.LLM_management.Tools.rag` at runtime, so this integration
# test patches that exact module namespace instead of the parallel
# the removed legacy RAG package copy.
runtime_rag = importlib.import_module("backend.Features.LLM_management.Tools.rag")
...
class TestBackendIntegration(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.answer_patch = patch(
            "backend.Features.LLM_management.Tools.rag.generate_answer",
            side_effect=self._fake_generate_answer,
        )
        self.answer_patch.start()

        self.client = TestClient(backend_run.app)
