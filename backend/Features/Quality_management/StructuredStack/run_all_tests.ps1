...
try {
    # Run the structured backend test suite through the project virtualenv so local
    # Conda/Python installations do not interfere with imports or dependencies.
    & $pythonExe -m unittest discover -s backend\Features\Quality_management\StructuredStack -v
}
finally {
    Pop-Location
}
